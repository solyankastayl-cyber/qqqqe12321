    ...
    fetch(url)
      ...
      .then((json) => {
        ...
        if ((symbol === 'SPX' || symbol === 'DXY') && json.ok && json.data) {
          ...
          // Extract matches from explain.topMatches (SPX) or matches (DXY)
          const assetData = json.data;
          const matches = assetData.explain?.topMatches || assetData.matches || [];
          ...
          overlayData = {
            matches: matches.map(m => ({
              ...
              // UNIFIED: Backend returns 0-100 (SPX) or 0-1 (DXY), normalize to 0-1
              similarity: m.similarity > 1 ? m.similarity / 100 : m.similarity,
              phase: m.phase,
              // Backend returns % values (SPX) or decimal (DXY), normalize
              ...
              // UNIFIED: Include normalized series for Replay canvas
              windowNormalized: m.windowNormalized || [],
              aftermathNormalized: m.aftermathNormalized || [],
            })),
            windowLen: assetData.chartData?.currentWindow?.normalized?.length || 60,
            stats: {
              matchCount: matches.length,
              avgSimilarity: (assetData.diagnostics?.similarity || 0) > 1 
                ? (assetData.diagnostics?.similarity || 0) / 100 
                : (assetData.diagnostics?.similarity || 0),
            },
            currentWindow: assetData.chartData?.currentWindow || { raw: [], normalized: [] },
            distributionSeries: assetData.chartData?.bands || assetData.bands || { p10: [], p25: [], p50: [], p75: [], p90: [] },
          };
        } else {
