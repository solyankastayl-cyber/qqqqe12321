...
function toPathPoints(
  ...
  const dates = buildForwardDates(startDate, pctSeries.length);
  return pctSeries.map((pct, i) => ({
    t: i,
    date: dates[i],
...
export async function buildDxySyntheticPack(
  ...
  return {
    ...
    focusLen,
    pct: {
      p10: p10.map(v => Math.round(v * 10000) / 10000),
      p50: p50.map(v => Math.round(v * 10000) / 10000),
      p90: p90.map(v => Math.round(v * 10000) / 10000),
      mean: meanPct.map(v => Math.round(v * 10000) / 10000),
    },
    bands,
    synthetic,
    hybrid,
